...
public class WMTSUtils {
    ...
    public static List<String> getLayerFormats(TileLayer layer) throws IOException {
        return layer.getMimeTypes().stream().map(MimeType::getFormat).collect(Collectors.toList());
    }

    public static List<String> getLayerFormatsExtensions(TileLayer layer) throws IOException {
        ...
        if (filters != null) {
            dimensions =
                    filters.stream()
                            .filter(
                                    filter ->
                                            !"STYLES".equalsIgnoreCase(filter.getKey())
