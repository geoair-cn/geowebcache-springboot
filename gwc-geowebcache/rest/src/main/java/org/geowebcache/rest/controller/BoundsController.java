...
public class BoundsController extends GWCController {
    ...
    public ResponseEntity<?> doGet(
        ...
        if (tl == null) {
            throw new RestException(layer + " 未找到", HttpStatus.NOT_FOUND);
        }

        GridSubset grid = tl.getGridSubset(srs);

        if (grid == null) {
            throw new RestException(layer + " 不支持 " + srs, HttpStatus.NOT_FOUND);
        }
        ...
        if (type.equalsIgnoreCase("java")) {
            ...
            throw new RestException("未知或缺少格式扩展名 : " + type, HttpStatus.BAD_REQUEST);
        }
    }
