...
public class ByteStreamController {
    ...
    ResponseEntity<?> doGet(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            String property = SpringUtil.getProperty("gwc.context.suffix", "");
            String subPath = "" ;
            if(GutilObject.isEmpty( property)){
                subPath = contextPath+ "/rest/web/";
            }else{
                subPath = contextPath+"/"+property+ "/rest/web/";
            }
...
//                            request.getPathInfo().substring("/rest/web/".length()), "UTF-8");
                    URLDecoder.decode(
                            request.getRequestURI().substring(subPath.length()), "UTF-8");
        ...
        try {
            ...
            return new ResponseEntity<Object>(
                    "无法为其创建 MimeType " + extension, HttpStatus.INTERNAL_SERVER_ERROR);
        }
