...
public class GWCController {
     ...
     * @throws RestException if the layer name is null or empty, if the layer does not exist, or if
     *     an error was encountered retrieving the layer from the dispatcher.
     */
    protected static TileLayer findTileLayer(String layerName, TileLayerDispatcher layerDispatcher)
            throws RestException {
        if (layerName == null || layerName.length() == 0) {
            throw new RestException("图层名称未知", HttpStatus.BAD_REQUEST);
        }
        // GWC supports using + instead of space in layer names.
        layerName = layerName.replace("+", " ");

        if (!layerDispatcher.layerExists(layerName)) {
            throw new RestException("图层不存在: " + layerName, HttpStatus.NOT_FOUND);
        }
        ...
        try {
            ...
            throw new RestException(
                    "服务器内部错误: " + gwce.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
        }
