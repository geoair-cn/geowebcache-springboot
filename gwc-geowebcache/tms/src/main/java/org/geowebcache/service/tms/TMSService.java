    ...
    public ConveyorTile getConveyor(HttpServletRequest request, HttpServletResponse response)
        ...
        if (possibleSplit.isPresent()) {
            ...
            try {
                ...
                if (mimeType == null) {
                    throw new HttpErrorCodeException(400, "不支持的请求format: " + fileExtension);
                }
            } catch (MimeException me) {
                throw new ServiceException("无法根据扩展名确定请求的格式 " + fileExtension);
            }
            try {
                TileLayer tileLayer = tld.getTileLayer(layerId);
                GridSubset gridSubset = tileLayer.getGridSubset(gridSetId);
                if (gridSubset == null) {
                    throw new HttpErrorCodeException(400, "不支持的切片方案请求: " + gridSetId);
                }
    ...
    public void handleRequest(Conveyor conv) throws GeoWebCacheException {
        ...
        if (paramsLength < 2) {
            ...
            if (!version.equals("1.0.0")) {
                throw new GeoWebCacheException("不支持的版本 " + version + ", 只有1.0.0版本受到支持.");
            } else {
