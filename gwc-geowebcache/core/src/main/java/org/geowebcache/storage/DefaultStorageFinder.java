...
/** 定位 GeoWebCache 文件缓存检查系统变量、servlet 上下文和环境变量的位置 */
    ...
    private void determineDefaultPrefix() {
        ...
        for (int i = 0; i < varStrs.length && defaultPrefix == null; i++) {
            for (int j = 0; j < typeStrs.length && defaultPrefix == null; j++) {
                ...
                if (!fh.exists()) {
                    log.error(msgPrefix + " , 但这条路不存在");
                    continue;
                }
                if (!fh.isDirectory()) {
                    log.error(msgPrefix + " , 这不是目录");
                    continue;
                }
                if (!fh.canWrite()) {
                    log.error(msgPrefix + " , 这是不可写的");
                    continue;
                }
        ...
        if (this.defaultPrefix == null) {
            ...
            if (tmpDir != null) {
                ...
                if (!temp.exists() && !temp.mkdirs()) {
                    throw new RuntimeException("无法创建 " + temp.getAbsolutePath());
                }
                this.defaultPrefix = temp.getAbsolutePath();
            } else {
                logMsg = "无法确定临时目录。继续处理未定义的结果。";
            }
        } else {
            switch (iVar) {
