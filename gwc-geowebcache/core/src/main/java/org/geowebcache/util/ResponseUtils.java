...
public final class ResponseUtils {
    ...
    public static void writeFixedResponse(
        ...
        if (contentType.contains("text")) {
            /*
             这里是解决字节流在 body里面转换的时候中文编码乱码问题。
             当你在这里不指定编码的时候 在 tomcat与 jetty 里面会自动对 body里面的直接内容使用 iso8859-1编码
             tomcat 参见：org.apache.catalina.connector.Response（setContentType（）方法）
             jetty参见：org.eclipse.jetty.server.Response（setContentType（）方法）
            */
        ...
        if (resource != null) {
            try (OutputStream os = response.getOutputStream();
                ...
                log.debug("产生io异常: " + ioe.getMessage() + "\n\n" + ioe.toString());
            }
        }
    }
    ...
    private static ByteArrayResource loadBlankTile(DefaultStorageFinder defaultStorageFinder) {
        ...
        if (blankTilePath != null) {
            ...
            if (fh.exists() && fh.canRead() && fh.isFile()) {
                ...
                if (fileSize == blankTile.getSize()) {
                    log.info("加载空白透明图，路径为 " + blankTilePath);
                } else {
                    log.error("加载透明空白图失败，路径为 " + blankTilePath);
                }

            } else {
                log.error("" + blankTilePath + " 不存在 或者无法重新加载.");
            }
        }
        ...
        if (blankTile == null) {
            try {
                ...
                log.info("加载 " + ret + " 透明图片 (expected 425).");
            } catch (IOException ioe) {
    ...
    public static void writeErrorPage(
        ...
        errorMsg =
                "<html>\n"
                        + ServletUtils.gwcHtmlHeader("../", "webcache系统错误")
                        + "<body>\n"
    ...
    public static void writePage(
        ...
        if (message != null) {
            ...
            res = new ByteArrayResource(message);
        }
