...
public class XMLConfiguration
    ...
    public void setDefaultValues(TileLayer layer) {
        ...
        if (layer instanceof WMSLayer) {
            ...
            try {
                ...
                log.error("无法解析代理 URL " + wl.getProxyUrl() + " ! 无需代理即可继续！", e);
            }
    ...
    private GeoWebCacheConfiguration loadConfiguration() throws ConfigurationException {
        ...
        try {
            try (InputStream in = resourceProvider.in()) {
                //  加载配置文件开始
                log.info("------------------加载配置文件开始------------------");
                GeoWebCacheConfiguration geoWebCacheConfiguration = loadConfiguration(in);
                log.info("------------------加载配置文件结束------------------");
    ...
    private GeoWebCacheConfiguration loadConfiguration(InputStream xmlFile)
        ...
        for (TileLayer layer : layers) {
            if (layer.getClass().getName().contains("ArcGISCacheLayer")) {
                arcGISCaches.put(layer.getName(), layer);
            }
        }
        //        LayerInfoSeo layerInfoSeo = new LayerInfoSeo();
        //        layerInfoSeo.setEnableIs("启用");
        //        layerInfoSeo.setArcLayerType("tc-arcgisLayer");
        //        List<LayerInfoDto> layerInfoDtos = layerInfoService.searchList(layerInfoSeo);
        //        if (GutilObject.isNotEmpty(layerInfoDtos)) {
        //            // 配置文件中存在的话,移除配置文件中的，使用数据库中的
        //            for (LayerInfoDto layerInfoDto : layerInfoDtos) {}
        //        }
        log.info("加载数据库中的图层配置结束");
    ...
    /**
     * 负责写出整个 GeoWebCache 配置对象的方法
     *
     * <p>throws an exception if it does not succeed
     */
    private void persistToFile() throws IOException {
        Assert.isTrue(resourceProvider.hasOutput(), "资源提供者必须有输出");
        ...
        log.info("将配置写入 " + resourceProvider.getId());
    }
    ...
    @Override
    public void setGridSetBroker(@Autowired @Qualifier("gwcGridSetBroker") GridSetBroker broker) {
