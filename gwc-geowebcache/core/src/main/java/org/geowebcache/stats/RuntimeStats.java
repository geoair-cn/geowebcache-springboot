...
public class RuntimeStats {
    ...
    public String getHTMLStats() {
        ...
        synchronized (bytes) {
            ...
            if (runningTime > 0) {
                str.append("<tbody>");
                str.append("<tr><th colspan=\"2\" scope=\"row\">服务启动于:</th><td colspan=\"3\">");
                str.append(
                ...
                str.append(
                        "<tr><th colspan=\"2\" scope=\"row\">总请求数量:</th><td colspan=\"3\">"
                                + totalRequests);
                str.append(" (" + totalRequests / (runningTime) + "/s ) ");
                str.append("</td></tr>\n");

                str.append(
                        "<tr><th colspan=\"2\" scope=\"row\">未处理的 WMS 请求总数:</th><td colspan=\"3\">"
                                + totalWMS);
                str.append(" (" + totalWMS / (runningTime) + "/s ) ");
                str.append("</td></tr>\n");

                str.append(
                        "<tr><th colspan=\"2\" scope=\"row\">总字节数:</th><td colspan=\"3\">"
                                + totalBytes);
                ...
                str.append("<tbody>");
                str.append("<tr><th colspan=\"5\">运行时统计数据尚不可用，请在几秒钟后重试</th></tr>");
                str.append("<tbody>");
            }

            str.append("<tr><th colspan=\"2\" scope=\"row\">缓存命中率:</th><td colspan=\"3\">");
            ...
            str.append("<tr><th colspan=\"2\" scope=\"row\">空白/KML/HTML:</th><td colspan=\"3\">");
            if (totalRequests > 0) {
                if (totalHits + totalMisses == 0) {
                    str.append("100.0% 的 请求");
                } else {
                    int rounded =
                            (int)
                                    Math.round(
            ...
            str.append("<tr><th colspan=\"2\" scope=\"row\">最高请求速率:</th><td colspan=\"3\">");
            if (totalRequests > 0) {
            ...
            str.append("<tr><th colspan=\"2\" scope=\"row\">最高带宽:</th><td colspan=\"3\">");
            if (totalRequests > 0) {
            ...
            str.append(
                    "<tr><th scope=\"col\">时间间隔</th><th scope=\"col\">请求数量</th><th scope=\"col\">速度</th><th scope=\"col\">字节</th><th scope=\"col\">带宽</th></tr>\n");
            ...
            str.append(
                    "<tr><td colspan=\"5\">数字延迟 "
                            + pollInterval
                            + " 秒 ,并不考虑http上面的数据传输时间开销</td></tr>");

            str.append("<tr><td colspan=\"5\">缓存命中率是模糊统计</td></tr>");
