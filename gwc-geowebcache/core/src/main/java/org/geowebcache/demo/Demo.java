...
public class Demo {
        ...
        if (action != null) {
            ...
            if (formatStr != null) {
                if (!layer.supportsFormat(formatStr)) {
                    throw new GeoWebCacheException("未知或者不支持的格式 " + formatStr);
                }
            } else {
    ...
    private static String generateHTML(
        ...
        buf.append("<html>\n");
        buf.append(ServletUtils.gwcHtmlHeader("", "webcache 演示"));
        buf.append("<body>\n");
        buf.append(ServletUtils.gwcHtmlLogoLink(""));
        buf.append(
                "<table cellspacing=\"10\" border=\"0\">\n"
                        + "<tr><td><strong>图层名称:</strong></td>\n"
        ...
        buf.append("<br />");
        buf.append(
                        "<strong>这仅仅是一些快速开始的演示效果. GeoWebCache 会支持一下几种能力:</strong><br />\n"
                                + "<ul><li>WMTS服务, TMS服务, 虚拟地球 和 谷歌地图</li>\n"
                                + "<li>代理OGC Wms 的  GetFeatureInfo, GetLegend 和一些其他的请求</li>\n"
                                + "<li>对一些请求与参数进行过滤</li>\n"
                                + "<li>输出格式调整，如压缩级别</li>\n"
                                + "<li>可调过期标头和自动缓存过期</li>\n"
                                + "<li>RESTful 接口 进行切图与一些配置</li>\n"
                                + "</ul>\n"
                                + "<br />\n"
                                + "<strong>重载 切片图层配置:</strong><br />\n"
                                + "<p>你可以按下面的按钮进行重新载入配置. </p>\n"
                                //                                + "The username / password is
                                // configured in WEB-INF/user.properties, or the admin "
                                //                                + " user in GeoServer if you are
                                // using the plugin.</p>\n"
                ...
                .append(
                        "\" method=\"post\">"
                                ...
                                + "<span><input style=\"padding: 0; margin-bottom: -12px; border: 1;\"type=\"submit\" value=\"重新载入图层配置\"></span>"
                                + "</form>"
                                + "<br /><strong>清空所有图层:</strong><br />\n"
                                + "<p>清空所有图层"
                                + "<form form id=\"truncate\" action=\"")
                .append(truncatePath)
                .append(
                        "\" method=\"post\"><input type=\"hidden\" name=\"<truncateAll>\" value=\"</truncateAll>\"/>"
                                + "<span><input style=\"padding: 0; margin-bottom: -12px; border: 1;background-color:LightCoral;\"type=\"submit\" value=\"清空所有瓦片缓存\"></span>"
                                + "</form><br />"
                                + "</body></html>");
    ...
    private static void tableRows(
        ...
        for (String layerName : layerList) {
            ...
            buf.append("<tr><td style=\"min-width: 100px;\"><strong>")
                    ...
                    .append("</strong><br />\n");
            buf.append("<a href=\"rest/seed/").append(layer.getName()).append("\">对图形进行切片</a>\n");
            buf.append("</td><td>");
            //  是否启用中文适配
            if (layer.isEnabled()) {
                buf.append("启用");
            ...
            buf.append("<td>");
            // 获取图层类型
            Class<? extends TileLayer> aClass = layer.getClass();
            ...
            buf.append("<td>");
            // 获取元数据信息
            // http://192.168.0.110:12307/geowebcache/geoair/layer/0929/wmts?REQUEST=getcapabilities

            if ("TC_XYZLayer".equals(string) || "TC_TMSLayer".equals(string)) {
                buf.append(
                        "<a href=\"geoair/"
                                + "xyz"
                                + "/"
            ...
            } else if (string.contains("TC_ArcGISCacheLayer")) {
                ...
                buf.append(
                        ...
                        "<a href=\"geoair/"
                                + "layer"
                                + "/"
                                + URLEncodeUtil.encode(
                                layer.getName(), CharsetUtil.CHARSET_ISO_8859_1)
                                + "/wmts?REQUEST=getcapabilities \">元数据</a>");
            } else {
                buf.append("<a href=\"service/wmts?REQUEST=getcapabilities\">元数据</a>");
            }
            buf.append("</td>");
            buf.append("<td><table width=\"100%\">");

            if ("TC_TMSLayer".equals(string) || "TC_XYZLayer".equals(string)) {
                for (String gridSetId : layer.getGridSubsets()) {
                    GridSubset gridSubset = layer.getGridSubset(gridSetId);
                    String gridSetName = gridSubset.getName();
                    if (!(gridSetName.contains("900913") || gridSetName.contains("3857")|| gridSetName.contains("4326"))) {
                        continue;
                    }
